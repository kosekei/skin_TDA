...
gs_result.to_csv('data/result_binary.txt', sep='\t', index=False)
...
df_importance.to_csv('data/variable_importance_binary.txt', sep='\t', index=False)

filename = 'data/model_binary.sav'
...
obs = obs_pred3["obs"]
pred = obs_pred3["pred"]
obs_pred3.to_csv("data/obs_pred_binary.txt", sep='\t', index=False)
...
eval2 = eval1.round(3)
eval2.to_csv("data/eval_binary.txt", sep='\t', index=False)
