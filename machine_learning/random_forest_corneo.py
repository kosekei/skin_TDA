...
df = pd.read_table("data/persistence_image_binary_wt_morph_sd1.txt")
df = df.replace('M', 0)
df = df.replace('F', 1)
...
gs_result.to_csv('data/result_binary_wt_morph_sd1_corneo.txt', sep='\t', index=False)
...
df_importance.to_csv('data/variable_importance_binary_wt_morph_sd1_corneo.txt', sep='\t', index=False)

filename = 'data/model_binary_wt_morph_sd1_corneo.sav'
...
obs = obs_pred3["obs"]
pred = obs_pred3["pred"]
obs_pred3.to_csv("data/obs_pred_binary_wt_morph_sd1_corneo.txt", sep='\t', index=False)
...
eval2 = eval1.round(3)
eval2.to_csv("data/eval_binary_wt_morph_sd1_corneo.txt", sep='\t', index=False)
